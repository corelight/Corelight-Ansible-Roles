...
```yaml
enable_cron_job:     false      # true or false (setting to false will remove an existing cron job)
...
```none
ansible-playbook -i common/vars/inventory.yml suricata-update.yml
```
...
```none
ansible-playbook -i common/vars/inventory.yml suricata-update.yml --extra-vars '{"target":"all"}'
```
...
```none
ansible-playbook -i common/vars/inventory.yml suricata-update.yml --extra-vars '{"target":"all"}' --extra-vars '@common/vars/secrets.yml'
```
